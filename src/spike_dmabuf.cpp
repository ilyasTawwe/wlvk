...
void Spike::registry_global(void* data, wl_registry* registry, uint32_t name,
                            const char* interface, uint32_t version) {
...
void Spike::wm_base_ping(void*, xdg_wm_base* wm_base, uint32_t serial) {
    xdg_wm_base_pong(wm_base, serial);
}

void Spike::xdg_surface_configure(void* data, xdg_surface* xdg_surface, uint32_t serial) {
    auto* self = static_cast<Spike*>(data);
...
void Spike::toplevel_close(void* data, xdg_toplevel*) {
    static_cast<Spike*>(data)->closed_ = true;
}

void Spike::frame_done(void* data, wl_callback* callback, uint32_t) {
    auto* self = static_cast<Spike*>(data);
...
VKAPI_ATTR VkBool32 VKAPI_CALL Spike::debug_callback(
...
void Spike::create_window() {
    ...
    xdg_toplevel_set_title(toplevel_, "spike-dmabuf");
    xdg_toplevel_set_app_id(toplevel_, "codotaku-spike-dmabuf");
    wl_surface_commit(surface_);
}

void Spike::poll_and_dispatch() {
...
void Spike::create_instance() {
    ...
    app_info.pApplicationName = "spike-dmabuf";
    ...
    messenger_info.pfnUserCallback = &Spike::debug_callback;
...
void Spike::pick_physical_device() {
    const char* required[] = {
...
wp_linux_drm_syncobj_timeline_v1* Spike::share_syncobj(uint32_t syncobj, int* out_fd) {
    int fd = -1;
...
void Spike::materialize_acquire_point(uint32_t syncobj, uint64_t point, VkFence fence) {
...
void Spike::feedback_tranche_formats(void* data, zwp_linux_dmabuf_feedback_v1*,
                                     wl_array* indices) {
    auto* self = static_cast<Spike*>(data);
...
void Spike::feedback_tranche_done(void* data, zwp_linux_dmabuf_feedback_v1*) {
    auto* self = static_cast<Spike*>(data);
...
void Spike::query_default_feedback() {
...
void Spike::choose_modifier() {
    ...
    std::fprintf(stderr, "spike: LINEAR unavailable; falling back to modifier 0x%016llx\n",
                 static_cast<unsigned long long>(chosen_modifier_));
}

void Spike::record_clear(uint32_t index, float r, float g, float b) {
